...
public:
    void pancake_sort_descending_burnt_up(int *p,int *sum) {
        int i, j, t;
        int a, b;
        bool f;
        int n1 = 0, n2 = 0;
...
public:
    ...
    void sort_exterior(int *p,int *sum) {
        for (i = 0; i < y; i++) {
            for (j = 0; j < x; j++) {
                if (i == 0 || i == y - 1 || j == 0 || j == x - 1) {
                    mpiles[i][j].pancake_sort_descending_burnt_up(p,sum);
                    cter++;
                }
            }
        }
